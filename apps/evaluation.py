# Modified from PIFuHD evaluator
...
def run(obj_root, target_root):
    ...
    for obj in tqdm(obj_list):  
        try:
            epoch = int(obj.split('_')[-2])
        except ValueError:
            epoch = 0
        index = int(obj.split('_')[-1].split('.')[0])
        obj_path = os.path.join(obj_root, obj)
        target_path = os.path.join(target_root, f'smplx_{str(index + 1).zfill(6)}.obj')     # target smpl models' indices start from 1
    ...
    print('chamfer: %.6f  p2s: %.6f ' % (vals[0], vals[1]))
    return vals[0]
...
if __name__ == '__main__':
    ...
    obj_root = args.root
    target_root = args.tar_path
    # import ipdb; ipdb.set_trace()
    run(obj_root, target_root)
