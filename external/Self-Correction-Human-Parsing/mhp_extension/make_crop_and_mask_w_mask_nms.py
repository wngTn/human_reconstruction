...
def make_crop_and_mask(img_info, pred, file_list, crop_save_dir, mask_save_dir, args):
    ...
    for instance in pred[img_id]['instances']:
        ...
        # print(mask.shape, panoptic_seg.shape)
        # if mask.shape[0] != panoptic_seg.shape[0]:
        #     continue
    ...
    # if not os.path.exists(lel:=os.path.join(mask_save_dir, mask_name)[:-4]):
        # os.makedirs(lel, exist_ok=True)
