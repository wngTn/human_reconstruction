...
class SyntheticDataset(Dataset):
    ...
    def visibility_sample(self, data_dict):
        ...
        if self.opt.visibility_sample:
            ...
            pts_depth = []
            for view in range(self.num_views):
                pts_depth.append(index(depth[view, :, :, :].unsqueeze(0), pts_xy[:, view, :, :],
                                       size=self.opt.loadSize))
    ...
    def load_cam_parameters(self, parameters, cam_num):
        ...
        d = 90
        rotation_matrix = np.array([[1, 0, 0, 0], [0, np.cos(np.deg2rad(d)), -np.sin(np.deg2rad(d)), 0],
                                    [0, np.sin(np.deg2rad(d)), np.cos(np.deg2rad(d)), 0], [0, 0, 0, 1]],
                                   dtype=np.float32)
    ...
    def load_render_data(self, data_dict):
        ...
        for cam_id in self.cameras:
            ...
            depth_path = os.path.join(self.DEPTH, f"cam_{cam_id}",
                                      f"r_{frame_id}_{cam_id}_depth_{str(frame_id).zfill(4)}.exr")
            if not os.path.exists(depth_path):
                depth_path = depth_path.replace(".exr", ".png")
            normal_path = os.path.join(self.NORMAL, f"cam_{cam_id}",
                                       f"r_{frame_id}_{cam_id}_normal_{str(frame_id).zfill(4)}.exr")
            if not os.path.exists(normal_path):
                normal_path = normal_path.replace(".exr", ".png")
            smpl_norm_path = os.path.join(self.SMPL_NORMAL, f"person_{person_id}", f"cam_{cam_id}",
                                          f'r_{frame_id}_{cam_id}_global_smpl.png')
            mask_path = os.path.join(self.MASK, f"person_{person_id}", f"cam_{cam_id}",
                                     f"r_{frame_id}_{cam_id}_segmentation_{str(frame_id).zfill(4)}.png")
