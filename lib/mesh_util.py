...
def gen_validation(opt, net, device, data, epoch, iteration, use_octree=True, threshold=0.5):
    ...
    extrinsic = data['extrinsic'].unsqueeze(0)
    vox_tensor = data['vox'].unsqueeze(0)
    smpl_normal = data['smpl_normal'].unsqueeze(0)
    save_smpl_normal = smpl_normal.clone()
    normal_tensor = data['normal']
    scale, center = data['scale'].unsqueeze(0), data['center'].unsqueeze(0)
    mask, ero_mask = data['mask'].unsqueeze(0), data['ero_mask'].unsqueeze(0)
...
def reconstruction_3d(net, cuda, calib_tensor, extrinsic, 
    ...
    def eval_func(points):
        ...
        error = net.get_error()[0][0]
        return pred.detach().cpu().numpy(), error.detach().cpu().numpy()

    # Then we evaluate the grid
    if use_octree:
        sdf, error = eval_grid_octree(coords, eval_func, num_samples=num_samples)
    else:
    ...
    verts = verts.T
    return verts, faces, normals, values, error
